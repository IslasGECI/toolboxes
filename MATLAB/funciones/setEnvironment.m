function setEnvironment(nombreAmbiente)
    esDiagonal = contains(nombreAmbiente,'/') || contains(nombreAmbiente,'\');
    assert(~esDiagonal,'El nombre del ambiente no puede contener diagonales')
    % Cambia el ambiente
    toolboxes.delEnvironmentFromMatlabPath()
    setEnvironmentName(nombreAmbiente);
    toolboxes.addEnvironmentToMatlabPath()
    if string(nombreAmbiente) == string(toolboxes.getDefaultEnvironmentName())
        setPrompt('>> ');
    else
        setPrompt(['(' nombreAmbiente ') >> ']);
    end
end
